...
- 已完成回库导出列表：
  ...
  - 展示列为商品信息、上新时间、回库时间、门店库存。
  - 支持按门店分别导出 Excel 文件，用户确认文件已成功保存后，将参与导出的商品标记为 `status = "exported"`。
- 已完成库存查询列表：
  - 页面新增 `库存查询` 切换项，列表默认空。
  - 用户输入建档开始日期和结束日期后，展示建档时间在自然日闭区间内且当前有正库存的商品。
  - 展示列为商品信息、建档时间、当前状态、门店库存，不展示操作列。
- 已完成业务参数设置入口：
...
- 回库导出列表支持 `导出Excel` 操作：
  - 按门店分别生成 Excel 文件，文件名格式为 `门店名称_回库列表_YYYYMMDD.xlsx`。
