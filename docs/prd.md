...
export interface ReminderSettings {
  ...
  forceReturnDays: number; // 强制回库时间，按天保存，默认 56
  forceReturnUnit: ReminderTimeUnit; // 强制回库时间在页面中的展示单位，默认 week
...
- 提醒列表切换区域最右侧展示 `刷新` 和 `参数设置` 入口。
- 点击 `刷新` 后重新读取本地数据库并更新当前列表、各列表数量、库存查询结果和回库导出按钮状态；不触发有赞报表导入。
- 点击 `参数设置` 后弹出参数设置框。
- 可设置参数：
...
- 读取参数时会做归一化：非法单位、非正提醒天数、负数最大推后次数都会回退到默认值。
- 修改参数后，三类提醒列表、状态流转校验和导入 workflow 的强制回库判断都按最新参数计算。
- 已经存在的 `listingRemindTime`、`transferRemindTime`、`returnRemindTime` 是绝对提醒时间，参数修改后不重算。

## Barcode 搜索

- `上新提醒`、`调货提醒`、`回库提醒` 和 `回库导出` 四个列表支持按商品条码搜索。
- 搜索使用输入值去除首尾空格后的字符串，对 `barcode` 做包含匹配；空搜索值显示当前列表全部商品。
- 搜索只在当前列表原本已经能够展示的商品中进行，不扩大提醒规则、状态规则或正库存门槛筛选范围。
- 四个列表分别保留各自的搜索词，切换列表后回显对应列表的搜索词。
- 搜索只影响当前表格展示结果，不影响各列表数量、回库导出按钮状态和回库导出范围。
- `库存查询` 列表不提供该 Barcode 搜索功能。

## 库存更新时间

- `调货提醒`、`回库提醒`、`回库导出` 和 `库存查询` 四个列表的搜索/操作栏左侧展示库存更新时间。
- 库存更新时间从当前 `stock` 表第一条记录的 `lastUpdatedTime` 字段读取，格式为 `YYYY-MM-DD HH:mm:ss`。
- `stock` 表为空、第一条记录缺少 `lastUpdatedTime` 或读取失败时，展示 `库存更新时间：-`。
- `上新提醒` 列表不展示库存更新时间。
...
- 商品报表字段映射：
  ...
  - 零售价 -> `costPrice`
  - 创建时间 -> `createdTime`
- 商品 `创建时间` 缺失或解析失败时，`createdTime` 使用导入执行日期的前一天 `23:59:59`。
- 新商品导入时：
  - `status` 初始化为 `pending`。
...
- `lastUpdatedTime` 使用库存报表导入合并时的当前时间。
- 库存数据是全量快照。
- 有库存报表 URL 时，只取 `task.extract_data` 解析、清洗、去重后的第一个有效 URL 作为最新库存快照。
- 最新库存报表未导入过时，先解析该 xlsx；解析成功后再清空并写入 `stock` 表。
- 最新库存报表已导入过时，本次库存导入全部跳过，保留旧库存，不回退导入更旧的未导入库存报表。
- 没有有效库存报表 URL 时，保留旧库存。
...
`门店库存` 展示规则与 `调货提醒列表` 一致；只展示 `stock > 0` 的库存记录。

### 过滤条件

- `status` 字段为 `returned`。
- 当前库存快照中存在同 `barcode` 且 `stock > 0` 的库存记录；无正库存记录时不显示在回库导出列表中。

### 操作

- 导出Excel：点击 `导出Excel` 按钮后，按门店分别生成 Excel 文件，并打包为一个 Zip 文件下载。
- Zip 文件名格式为 `回库列表_YYYYMMDD.zip`，其中 `YYYYMMDD` 为操作当天日期。
- Zip 内每个门店 Excel 文件名格式为 `XXXX_回库列表_YYYYMMDD.xlsx`，其中 `XXXX` 为门店名称。
- 每个门店 Excel 文件只包含该门店的回库商品库存记录；同一商品存在多个门店正库存时，分别写入对应门店文件。
...
- 库存查询列表默认为空。
- 用户需要输入建档开始日期和建档结束日期后点击 `查询`。
- 查询范围按自然日闭区间计算：开始日期 00:00:00 至结束日期 23:59:59。
- 开始日期和结束日期必须同时填写；结束日期不能早于开始日期。
- 日期必须是真实自然日；例如 `2026-02-31`、`2026-13-01`、`2026-00-01` 会被拒绝并提示 `日期格式无效`。
...
- `createdTime` 在用户输入的建档起止日期范围内。
- 不限制商品 `status`。
- 当前库存快照中存在同 `barcode` 且 `stock > 0` 的库存记录；无正库存记录时不显示在库存查询列表中。

### 操作

- 库存查询列表仅用于查看，不展示操作列，不提供状态流转或推后提醒操作。
...
### Q：参数设置会影响哪些业务计算？
A：参数设置会影响三类提醒列表的首次进入时间、调货/回库推后按钮显隐和动作校验，以及导入 workflow 中的强制回库判断。周统计 workflow 当前未启用。
...
### Q：页面是否需要一次性读取全部库存？
A：不需要。页面只在 `调货提醒列表`、`回库提醒列表`、`回库导出列表` 和 `库存查询列表` 需要展示门店库存时，按当前列表商品的 `barcode` 查询库存。
...
- 当前 `src/workflows/weekly_stats_workflow.ts` 整体保持注释状态，不会注册或执行定时 workflow。
- `src/libs/weekly_stats.ts` 保留周统计纯函数能力，供后续重新启用 workflow 或页面展示时复用。
- 如果后续重新启用周统计，建议每周一凌晨 01:00 统计一次，`统计周期` 为上周一 00:00 至本周一 00:00 的半开区间，页面文案展示为上周一 00:00 至上周日 23:59。
- 计算“统计周期内待处理提醒商品数”时，提醒列表过滤条件中的“当前时间”固定为 `统计周期` 结束时间，而不是统计任务实际执行时间。
- 周统计纯函数支持传入参数设置；没有传入时使用默认参数。
